...
func run() -> void:
	...
	for island: int in [1, 2, 3]:
		...
		check(game.hud._refs.has("activity:duck") and not game.hud._refs["activity:duck"].disabled, "island %d offers usable shared duck training" % island)
		check(game.hud._refs["activity:duck:detail"].text.begins_with("%d duck" % island), "island %d shows correct flock size" % island)
		game.hud._refs["activity:duck"].pressed.emit()
		check(game.activities.duck_level == island, "island %d training button upgrades the same global rank" % island)
